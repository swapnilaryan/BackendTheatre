...
const insertData = (req, res, next, isAPI) => {
	...
	let values = [req.body.imdbID, req.body.movieTitle,
		...
		JSON.stringify(req.body.genre), req.body.movieURL];
		...
		if (err) {
			...
			connection.query(query, function (err, rows) {
				if (err) {
					...
					if (rows[0].mtURL) {
						// call the crawl on every visit. Not good for scaling.
						let request = {
							body: {
								'movieURL': rows[0].mtURL,
								'imdbID': rows[0].mtImdbID,
								'isAPI': false
							}
						};
						crawlData(request, res, false);
					}
